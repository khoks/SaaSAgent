...
### [2026-05-10] Compose-cycle-id causality binding: server-composed UI artifacts linked to client emits by a monotonic cycle token
- **Originator:** Rahul / joint (reduced to practice in Phase 1.1–1.2)
- **Source:** Every `LayoutTree` broadcast carries a `composeCycleId` (UUID v4 + timestamp); every `InstructionEnvelope` the shell emits back includes the `composeCycleId` of the layout currently visible to the user. Verified live in Chrome (2026-05-10): runtime logs showed all DOM events, thumbs-up, and implicit re-ask envelopes tagged with the correct cycle id of the layout that was on-screen.
- **Description:** In server-driven AI agent UIs, the server composes a new layout artifact per conversation turn; the client renders it and the user may interact with it. Without a causality token, there is no runtime link between a user click/message/feedback and the specific layout version that prompted it. The compose-cycle-id solves this by (a) the server minting a stable id for each composed layout and including it in the SSE event, (b) the shell tracking the id of the currently-visible layout in memory, (c) every client → server envelope (instruction, feedback, implicit signal) echoing that id. The runtime can therefore attribute any client action to the exact server-composed artifact that generated the context for that action.
- **Prior-art assessment:** SSE `id:` field is for reconnection resumption (different semantics). React `key` prop is renderer-local. Anthropic `tool_use` block ids are per-tool-call, not per-rendered-UI. No prior art attaches a causality token to a server-composed AI UI artifact and mandates that all client emits echo it.
- **Novelty signal:** high — uncommon in production and unfamiliar in the AI agent UI literature.
- **Patentability hint:** strong — see `docs/patents/disclosures/P-001-compose-cycle-id-causality.md`. Method claim: (1) server mints composeCycleId per composed layout, (2) client tracks the currently-visible cycleId, (3) every client emit includes the cycleId, (4) runtime uses it for eval attribution, churn signal weighting, and memory replay.
- **Open questions:** How long to retain dead cycle ids in the runtime's session map? Should cycle ids survive page reload (session resume)?

### [2026-05-10] Prefix-discriminated three-tier capability routing for LLM tool-use APIs
- **Originator:** Joint (Rahul defined three-tier model in ADR-021; tool-mapper prefix scheme implemented in Phase 2.1b)
- **Source:** `packages/runtime/src/planner/tool-mapper.ts` — names presented to Sonnet as `skill__<name>`, `tool__<name>`, `subagent__<name>`; dispatch reads the prefix to route to `SkillExecutor`, `ToolExecutor`, or `SubAgentExecutor` respectively.
- **Description:** LLM tool-use APIs expose a flat list of tool definitions to the model. Real production agents need to invoke three architecturally distinct tiers (in-process skills, HTTP tools, federated sub-agents) with different cost, latency, and failure semantics. Rather than hiding the tier from the model (routing purely at dispatch time, giving the model no cost signal) or using multiple tool lists (unsupported by most APIs), this design **name-encodes the tier as a fixed-length prefix** (`skill__`, `tool__`, `subagent__`) in each tool's name field. The model learns from the prefix which tier a capability lives in and can factor that into its reasoning. The runtime strips the prefix at dispatch time to determine the correct executor. No API contract change required.
- **Prior-art assessment:** LangChain/AutoGen/CrewAI present flat tool lists; no explicit tier encoding. Anthropic computer-use tool names don't carry tier information. Name-prefix routing in traditional message queues exists but is not applied to LLM tool-name dispatch.
- **Novelty signal:** medium-high — the combination of model-visible tier signal (via name prefix) + runtime dispatch (strip prefix, select executor) in a single mechanism is non-obvious.
- **Patentability hint:** possible — see `docs/patents/disclosures/P-002-prefix-discriminated-tool-routing.md`. Defensive publication may be more strategic than filing given how lightweight the implementation is.
- **Open questions:** Does the model reliably use the prefix to choose the cheapest adequate tier? Should the prefix schema be standardized across providers?

### [2026-05-10] Symmetric multi-agent federation: every runtime serves as both parent and sub-agent via a single `/federate` endpoint
- **Originator:** Joint (ADR-021 defined federated sub-agents; Phase 2.4.x added `/federate` to every runtime)
- **Source:** Phase 2.4.x implementation: `packages/runtime/src/transport/server.ts` — `POST /federate` endpoint that synthesizes a `user-message` InstructionEnvelope, runs the planner, and returns the result as a `FederationResponse`. Verified live: two separate runtimes (ports 8080 and 8081) where the parent delegated to the child via HTTP; the child ran its own SonnetPlanner + ToolExecutor.
- **Description:** Traditional multi-agent topologies are asymmetric: a hub agent implements orchestration APIs, specialist agents implement worker APIs. This design adds a `POST /federate` endpoint to EVERY runtime instance using EXACTLY the same code path as the user-facing WebSocket handler — the runtime receives a federation request, synthesizes it into an internal user-message envelope, runs its own planner pipeline, and returns the result. The role of "parent" or "child" is purely determined by which runtime initiates the HTTP call. Any runtime can be parent-of-another, child-of-another, or both simultaneously. There is no separate agent class, no hub binary, no role registration.
- **Prior-art assessment:** AutoGen, CrewAI, LangChain all use asymmetric multi-agent topologies with separate class hierarchies. IETF agent interoperability drafts propose protocols without mandating symmetric implementation. Anthropic MCP defines tool/resource discovery but not symmetric agent delegation.
- **Novelty signal:** high — the "every runtime is simultaneously a potential parent and a potential child with zero additional code" property is uncommon.
- **Patentability hint:** possible — see `docs/patents/disclosures/P-003-symmetric-federation-contract.md`.
- **Open questions:** Cycle detection when A→B→A delegation loops occur? Per-federation auth token vs. session-level mTLS?

### [2026-05-10] Implicit negative-signal inference via timing window on follow-up natural-language input
- **Originator:** Joint (deduced feedback concept from ADR-016/ADR-023; timing-window rule implemented in Phase 2.5.x)
- **Source:** Phase 2.5.x: `packages/runtime/src/transport/server.ts` (handleWsConnection — `implicitReaskWindowMs` default 8000ms); verified live in Chrome: `evalSignalCount: 1 → 2` after a second user message within the window.
- **Description:** When a user sends a new natural-language input within a configurable time window (default 8 seconds) AFTER the runtime has broadcast a composed layout for the PREVIOUS user turn, the runtime automatically records a `negative/user-implicit` eval signal attributed to the PREVIOUS layout's compose-cycle-id. The insight: if a user is satisfied with the agent's response, their next action is domain-specific (scrolling, clicking, completing a task); if they are NOT satisfied, the most likely next action is re-asking in natural language. The timing window is the discriminator. No explicit feedback widget is required; the inference is zero-friction for the user.
- **Prior-art assessment:** Click-through rate / dwell time as implicit signals (well-known). Conversation abandonment as negative signal (chatbot literature). Re-query analysis at the search-query level (not applied to AI-generated UI artifacts). No prior art uses a fresh NL input within a timing window to infer negative quality on a specific preceding AI-composed UI artifact.
- **Novelty signal:** high — the specific combination of (timing window) + (NL input as discriminator) + (attribution to specific prior compose-cycle) is uncommon.
- **Patentability hint:** strong — see `docs/patents/disclosures/P-004-implicit-reask-signal-inference.md`.
- **Open questions:** Optimal timing window (is 8s universal across domains? mobile vs. desktop?)? How to distinguish a re-ask from an independently new topic?

### [2026-05-10] Closed-loop explainable churn-risk derivation with per-feature contribution text
- **Originator:** Rahul (concept in ADR-031/ADR-016); joint (WeightedFeatureChurnCalculator + trainChurnWeights implemented in Phase 2.6.x and Bucket C.4)
- **Source:** Phase 2.6.x + Bucket C.4: `packages/runtime/src/churn/weighted-feature.ts` (parameterized sigmoid); `packages/runtime/src/churn/training.ts` (pure-numeric logistic regression, L2, warm-start, deterministic seed). Verified live: `/churn/<sessionId>` returned `risk: 'high'` with `factors: ["3 negative signals in session", "45.5% negative signal rate"]`.
- **Description:** The churn calculator outputs not just a 0–1 risk score but also a structured `factors: string[]` array explaining, in human-readable text, which input signals fired with what magnitude. The factors are generated **at inference time** by the parameterized linear model itself (not post-hoc via SHAP or LIME), because each weight in the model directly corresponds to a named signal type. The result: a single forward pass produces both the numeric risk and its English-language explanation simultaneously, with zero additional compute. The `trainChurnWeights()` function refines those weights from observed eval signals using pure-numeric gradient descent (L2 regularization, warm-start, deterministic seed for reproducibility), closing the loop from VoC signals → model weights → planner suppression decisions → new interactions → new signals.
- **Prior-art assessment:** LightGBM/XGBoost produce feature importances post-hoc; SHAP is computationally expensive. TabNet has built-in attention attribution but not human-readable text. Simple logistic regression has per-feature coefficients (well-known) but is not packaged as a first-class agent feedback-loop component with warm-start training and text factor generation.
- **Novelty signal:** medium-high — the specific packaging (agent feedback loop + simultaneous score+text, warm-start training from eval signals) is the novel combination.
- **Patentability hint:** possible — see `docs/patents/disclosures/P-005-explainable-churn-derivation.md`.
- **Open questions:** When to graduate from linear model to LightGBM? Feature engineering beyond signal counts (dwell time, feature-specific signal)?
