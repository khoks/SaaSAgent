...
### [2026-05-11] `ComposedLayoutMetadata` as a zero-protocol-churn side-channel for runtime state
- **Originator:** joint (emerged from Phase 7 QuotaStatus design)
- **Source:** Phase 7 ADR-036 positive consequences: "The `metadata.quotaStatus` mechanism is reusable: future 'session warning' or 'feature-flag' payloads can ride the same channel without protocol churn." Session 2026-05-11.
- **Description:** Instead of adding new WebSocket message types or new SSE event names each time the runtime needs to communicate per-session state to the shell, all such state travels as typed fields on `ComposedLayoutMetadata` — the metadata envelope already attached to every composed layout broadcast. `QuotaStatus` was the first occupant; the pattern generalizes to any runtime-state signal that changes at layout-granularity: feature flags in effect for this session, active A/B test bucket, session health warning, auth-expiry countdown, rate-limit proximity. The shell reads the fields it knows about and ignores unknown ones; adding a new runtime-state signal only requires (1) a new optional field in the protocol's `ComposedLayoutMetadata` type and (2) a new shell consumer — no new message types, no new event names, no version negotiation. The metadata flows to the shell on every layout anyway, so the marginal cost of additional fields is nearly zero.
- **Prior-art assessment:** HTTP response headers serve a similar side-channel role for per-response state (rate-limit headers, cache-control, etc.). GraphQL extensions and gRPC trailers provide side-channel fields in structured protocols. The specific application to a **bidirectional agent loop** — where composed interactive artifacts already carry structured metadata, and that metadata is the designated extensibility vehicle for all runtime-to-shell state signals — is a natural but non-obvious design choice that prevents protocol proliferation as the feature set grows.
- **Novelty signal:** low — the pattern is analogous to HTTP response headers and has clear prior art in structured protocol design. The value is in the deliberate application to agent-loop artifact metadata.
- **Patentability hint:** none — this is an application of a standard extensible-metadata pattern; prior art is broad.
- **Open questions:** Maximum metadata envelope size? Versioning when a field's type changes? Security considerations if runtime-state signals leak sensitive billing info to a compromised shell?
