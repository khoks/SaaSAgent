...
### [2026-05-12] Multi-signal proactive scoring + attention budget contract (P-006 candidate)
- **Originator:** joint (emerged from Phase 5 ProactiveEngine implementation)
- **Source:** ADR-043 (Phase 5): "Patent-novelty surface: the multi-signal scoring + budget contract is a P-006 candidate (companion to P-005 explainable churn). The per-signal factors array is the same explainability mechanism." Verified live 2026-05-12: "Engine logged 2 fires (matching the budget=2 cap) with full per-signal explainability."
- **Description:** The proactive agent trigger mechanism combines two novel elements: (1) a multi-signal weighted-sum scorer that derives a `ProactiveScore` from five server-side per-WS state signals (plannerConfidence, memoryMatch, workflowContinuity, domRelevance, timeSinceLastTouch), each contributing a named factor to a structured `factors[]` array for explainability; (2) an atomic token-bucket `AttentionBudget` that enforces a per-session fire cap — budget is consumed only on actual fires (score ≥ threshold AND `tryConsume` returns true), preventing below-threshold scoring ticks from burning allocation. The combination of (a) server-side multi-signal derivation from WS-level state, (b) per-signal explainable factors, (c) atomic attention budget enforced downstream of scoring, (d) cooldown window reusing the Phase 2.5 implicit-reask window — as a unified proactive-trigger protocol in an embeddable real-time agent — appears uncommon in the literature and in production.
- **Prior-art assessment:**
  - Recommendation systems use multi-armed bandits and contextual bandits to decide when to surface items — but these operate on catalog items, not on agent-generated conversational UI broadcasts.
  - Push notification systems (mobile) use engagement scoring + frequency caps but are coarser-grained (no per-session typed-artifact attribution, no causality chain).
  - Conversational AI platforms (Intercom, Drift) have "proactive messages" triggered by rule-based conditions (URL, time-on-page) — not a multi-signal scored + budget-constrained mechanism.
  - The combination of (a) typed-JSON layout broadcast as the proactive output, (b) `composeCycleId` on proactive layouts (same as reactive, enabling identical eval attribution via P-001/P-004), (c) pluggable `ProactiveScorer` + `AttentionBudget` interfaces, (d) `suggestedIntent` per-deployment customization — as a packaged runtime primitive in an embeddable SaaS agent — appears novel.
- **Novelty signal:** medium-high — scoring + budget themselves are known patterns; the specific integration contract (per-WS state → scorer → budget → typed-layout broadcast via SSE, with pluggable interfaces and per-signal explainability) is a distinctive design in this context.
- **Patentability hint:** possible — method claim covering: (1) deriving a multi-signal proactive score from per-session server-state signals, (2) enforcing an atomic per-session attention budget with threshold-gated consumption, (3) broadcasting a typed-JSON composed layout via SSE on fire (indistinguishable from reactive layouts, same cycle-id tracking), (4) per-signal explainability in a `factors[]` array. Recommend P-006 formal disclosure before OSS publish per `docs/patents/FILING-CHECKLIST.md`.
- **Open questions:** P-006 disclosure with counsel: single filing with P-005 (explainable scoring) or separate? Two-band cooldown (< 3s post-broadcast suppresses proactive to avoid race with reactive)? ML-learned signal weights (LightGBM v1 upgrade — see future-requirements).
