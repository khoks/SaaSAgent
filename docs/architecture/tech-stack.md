...
| Memory provider chain (in-process) | NullMemory / KeyValue / File / Postgres / Chained | **NullMemoryProvider → KeyValueMemoryProvider (in-mem) → DurableFileMemoryProvider (atomic JSON-on-disk, temp-file + rename) → PostgresMemoryProvider (injectable `pg` client, deps-free interface) + ChainedMemoryProvider (reads from first-in-chain, writes to all)** — all behind `MemoryProvider` interface | [ADR-008](../decisions/decision-log.md), [ADR-041](../decisions/decision-log.md) |
| Churn risk model (current build) | Parameterized linear model / LightGBM / pluggable | **WeightedFeatureChurnCalculator: score = sigmoid(w1·negative_rate + w2·re_ask_rate + w3·short_session_rate); configurable weights + threshold; stepping stone until Python SDK + LightGBM ready** — behind `ChurnRiskCalculator` interface | [ADR-031](../decisions/decision-log.md), [ADR-041](../decisions/decision-log.md) |
| Transport hardening (runtime) | Open / bearer token + rate-limit | **Bearer auth via `SAAS_AGENT_AUTH_TOKEN` env var; WS uses `?token=` query-param fallback (browser WebSocket API cannot send Authorization header on upgrade); token-bucket `RateLimiter` keyed by client IP; opt-in (off when env var absent)** | [ADR-042](../decisions/decision-log.md) |
| Container packaging | Dockerfile only / docker-compose / Helm | **Dockerfile + Dockerfile.demo-host + docker-compose.yml (Phase 2.7); Helm chart deferred to v1 per ADR-026; entrypoint reads env vars into RuntimeConfig** | [ADR-026](../decisions/decision-log.md), [ADR-042](../decisions/decision-log.md) |
