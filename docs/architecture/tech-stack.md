...
| Sub-Agent federation topology | Hub-and-spoke / symmetric peer | **Symmetric: every runtime exposes `/federate` endpoint — acts as both orchestrator (via Sub-Agent registry) and sub-agent target; dynamic topology via registry contents** | [ADR-043](../decisions/decision-log.md) |
| EvalProvider implementation (MVP) | In-memory / ClickHouse-backed / pluggable | **`KeyValueEvalProvider` (in-memory, queryable by sessionId + kind) at MVP; `EvalSignal` intercepted on WS before planner; ClickHouse-backed at v1** | [ADR-044](../decisions/decision-log.md) |
| Deduced feedback — re-ask threshold | None / fixed 8s / configurable | **8-second window: `user-message` arriving ≤ 8s after last layout broadcast → auto-records `negative/user-implicit` on prior `composeCycleId`; host-configurable** | [ADR-045](../decisions/decision-log.md) |
| ChurnCalculator implementation (MVP) | Rule-based / weighted linear / LightGBM / pluggable | **Pluggable `ChurnCalculator` interface; `RuleBasedChurnCalculator` (Phase 2.6) + `WeightedFeatureChurnCalculator` sigmoid linear model (Phase 2.6.x) bundled; LightGBM at v1 (ADR-031)** | [ADR-046](../decisions/decision-log.md) |
| Runtime auth + rate limiting | None / bearer / OAuth2 / mTLS | **Bearer token auth gate (REST `Authorization` header + WS `?token=` query param) + token-bucket rate limiting (per-IP REST default 100 req/s; per-conn WS default 10 msg/s); configurable via `RuntimeConfig`** | [ADR-047](../decisions/decision-log.md) |
