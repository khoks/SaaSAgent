...
| End-user tier/quota system | None / fixed / **configurable** | **Configurable per-tier quotas + per-user tracking + visible "X requests remaining" element**. Phase 7 implementation: `TierProvider` interface + `NoQuotaProvider` (default, no banner) + `InMemoryTierProvider` (session-grain, UTC-day reset) + `QuotaBanner` shell widget (fine/warning/exceeded states). | [ADR-019](../decisions/decision-log.md), [ADR-040](../decisions/decision-log.md) |
...
| Runtime API auth (machine-to-machine) | mTLS only / shared bearer / JWT / pluggable | **Pluggable `AuthProvider`: `NoAuthProvider` (dev) + `BearerTokenAuthProvider` (shared secret, constant-time compare) + `JWTAuthProvider` (HS256/RS256, optional audience+issuer+clock-skew, pure `node:crypto`)** | [ADR-041](../decisions/decision-log.md) |
| Rate limiting (runtime API) | None / per-IP fixed window / token-bucket | **Token-bucket per IP** (configurable `capacity` + `refillRate`); applied after auth | derivative of [ADR-041](../decisions/decision-log.md) |
| E2E test framework | Playwright / Cypress / Puppeteer | **Playwright** (`@saasagent/e2e` package; 5 demo-ecommerce smoke tests; headless Chromium) | derivative of [ADR-036](../decisions/decision-log.md) |
