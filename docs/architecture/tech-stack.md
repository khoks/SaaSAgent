...
| Unit / integration test framework | Vitest, Jest | **Vitest** (ESM-native; fast; JSDOM available via `@vitest/environment-jsdom` for Web Component shell tests) | — |
| E2E test framework | Playwright, Cypress, Puppeteer | **Playwright** (`packages/e2e` / `@saasagent/e2e`; 5 smoke tests for demo-ecommerce at MVP) | — |
| Browser bundler (demo + shell) | Vite, webpack, esbuild, Parcel | **Vite** (dev-server for `apps/demo-host`, `apps/demo-ecommerce`, `apps/demo-travel`; fast HMR; ESM-native) | — |
| Automated browser testing (dev loop) | Playwright headful, Chrome DevTools, MCP preview | **`Claude_Preview` MCP** (`preview_start`, screenshot, snapshot, click, console_logs, network) for Claude-Code-driven browser verification during development | — |
| Runtime auth (MVP) | Bearer token, mTLS, OAuth2/OIDC, none | **Bearer token** (`SAAS_AGENT_TOKEN` env var; `Authorization: Bearer <token>` header; `?token=` for WebSocket clients) | [ADR-041](../decisions/decision-log.md) |
| Rate limiting (MVP) | Token bucket, fixed window, leaky bucket, none | **Token bucket per source IP** (configurable burst + fill-rate via `RuntimeConfig`; default 60 req/min burst 10) | [ADR-041](../decisions/decision-log.md) |
| AuthProvider abstraction (pluggable) | NoAuth, Bearer, JWT (HS256/RS256), OAuth2 | **Pluggable `AuthProvider` interface** shipped with `NoAuthProvider`, `BearerAuthProvider`, `JwtAuthProvider` (HS256 + RS256) implementations | derivative of [ADR-041](../decisions/decision-log.md) |
| Telemetry abstraction | OpenTelemetry, Datadog direct, custom | **Pluggable `Telemetry` interface** with `ConsoleTelemetry` (dev default) + `OpenTelemetryAdapter` (no hard OTel dep — uses dynamic import) | derivative of [ADR-030](../decisions/decision-log.md) |
| Metering | Custom, Stripe Metering API, cloud billing | **`MeteringProvider` interface** with `NoopMeteringProvider`, `ConsoleMeteringProvider`, and `UsageMeteringProvider` (in-memory rollup per tenant + operation, snapshot/reset) | derivative of [ADR-019](../decisions/decision-log.md) |
| Multi-tenant isolation (registries) | Single-tenant, per-tenant namespacing, separate DBs | **Per-tenant registry maps** — `MultiTenantSkillRegistry`, `MultiTenantToolRegistry`, `MultiTenantFeatureRegistry`, `MultiTenantSubAgentRegistry`; all keyed by `tenantId`; `tenantScopedSessionId(tenantId, sessionId)` helpers | derivative of [ADR-006](../decisions/decision-log.md) |
| Churn model training | Batch retraining, online learning, external ML platform | **`trainChurnWeights()`** — pure-numeric logistic regression, L2 regularization, warm-start, deterministic seed; runs in-process against accumulated eval signals | [ADR-031](../decisions/decision-log.md) |
